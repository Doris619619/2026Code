#include "rm_serial_ext/rm_serial_ext_down.hpp"

namespace ext_serial_driver
{
    DOWNSerialDriver::DOWNSerialDriver(const rclcpp::NodeOptions &options)
        : Node("rm_serial_ext_down", options), port_{new Port(2)}
    {
        RCLCPP_INFO(get_logger(), "Start DOWNSerialDriver!");

        port_->getParams("/dev/ttyACM1", 115200, "none", "none", "1");
        ...
        cmd_gimbal_joint_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
            "cmd_gimbal_joint", 10,
            std::bind(&DOWNSerialDriver::cmdGimbalJointCallback, this, std::placeholders::_1));

        cmd_tracking_sub_ = this->create_subscription<auto_aim_interfaces::msg::Target>(
            "tracker/target", 10,
            std::bind(&DOWNSerialDriver::visionTargetCallback, this, std::placeholders::_1));
        
        cmd_shoot_sub_ = this->create_subscription<example_interfaces::msg::UInt8>(
            "cmd_shoot", 10,
            std::bind(&DOWNSerialDriver::cmdShootCallback, this, std::placeholders::_1));
        ...
        try
        {
            ...
            if (!port_->serial_driver_->port()->is_open())
            {
                ...
                port_->receive_thread_ = std::thread(&DOWNSerialDriver::receiveData, this);
                // LOG
    ...
    void DOWNSerialDriver::receiveData()
    {
    ...
    //...
    void DOWNSerialDriver::cmdGimbalJointCallback(
      ...
      void DOWNSerialDriver::visionTargetCallback(
      ...
      void DOWNSerialDriver::cmdShootCallback(const example_interfaces::msg::UInt8::SharedPtr msg)
      {
        try
        {
            SendPacket packet;
...
RCLCPP_COMPONENTS_REGISTER_NODE(ext_serial_driver::DOWNSerialDriver)
